...
def chunks(text, max_tokens=4096):
    ...
    for j in range(len(text)):
        ...
        chunk = [text[j][i:i + max_tokens]
                 for i in range(0, len(text[j]), max_tokens)]
        chunks += chunk
...
def ui_app():
    ...
    st.set_page_config(page_title="PDF Summ")
    ...
    # Variable Slidera
    max_tk_val = st.slider("Select Max. Output Token :",
                           min_value=10, max_value=200, value=100)
    temp_val = st.slider("Select Temperature :",
        ...
        if file is not None:
            ...
            with st.spinner(text="Generating Summary..."):
                ...
                for i, t_chunk in enumerate(text_chunks):
                    ...
                    # Generate summary
                    summ_txt_val = summ_text(
                        txt=t_chunk, temp=p_temp, max_tokens=p_max_tk, p=p_top_p, n=p_n)
